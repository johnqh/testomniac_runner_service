...
export async function extractForms(
  ...
  return adapter.evaluate(() => {
    // All helpers must be inlined — adapter.evaluate() serializes this closure
    // into the browser context where Node.js module-scope functions aren't available.
    ...
    function buildField(el: Element, documentRef: Document): FieldInfo {
      ...
      const labelledByText = ariaLabelledBy
        ? ariaLabelledBy
            .split(/\s+/)
            .map(
              id => documentRef.getElementById(id)?.textContent?.trim() || ""
            )
            .filter(Boolean)
            .join(" ")
        : "";
    ...
    function dedupeBySelector(fields: FieldInfo[]): FieldInfo[] {
    ...
    function collectFields(
      ...
      return dedupeBySelector(
        fields.map(field => buildField(field, documentRef))
      );
    }
    ...
    function inferMethod(fields: FieldInfo[]): string {
      return fields.some(field => /^(search)$/i.test(field.type))
        ? "GET"
        : "POST";
    }
    ...
    function buildFormInfo(
      ...
      return {
        ...
        method: (
          form.getAttribute("method") || inferMethod(fields)
        ).toUpperCase(),
        fields,
    ...
    function groupOrphanControls(controls: Element[]): Element[][] {
      ...
      for (const control of controls) {
        const container =
          findGroupingContainer(control) ?? control.parentElement;
    ...
    function dedupeForms(forms: FormResult[]): FormResult[] {
      const seen = new Set<string>();
